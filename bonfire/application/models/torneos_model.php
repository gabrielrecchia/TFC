...
//        return $query->result_array();
        $query = $this->db->query("SELECT par.id,par.idtorneo,tor.nombre,par.fecha,eq1.nombre as equipo1,eq2.nombre as equipo2, par.idsede FROM tfc_partido par join tfc_equipo eq1 on par.idequipo1=eq1.id join tfc_equipo eq2 on par.idequipo2=eq2.id join tfc_torneo tor on par.idtorneo = tor.id  where par.jugado=0 and par.fecha <> '0000-00-00 00:00:00' and par.fecha between DATE(now()) and DATE_ADD(now(),INTERVAL 7 DAY) order by tor.id,par.fecha");
        $mapa = array();
        foreach ($query->result_array() as $partida){
            if (!array_key_exists($partida['nombre'], $mapa)) {
                $mapa[$partida['nombre']] = array();
                array_push($mapa[$partida['nombre']], $partida);
            }else{
                array_push($mapa[$partida['nombre']], $partida);
            }
        }
